...
export const PronunciationTrainer: React.FC<PronunciationTrainerProps> = ({ onFinish }) => {
  ...
  const startListening = () => {
    ...
    recognition.onresult = (event: any) => {
      ...
      const allTranscripts: string[] = Array.from(event.results[0]).map((r: any) => normalize(r.transcript));
            ...
            <div className="mt-6 grid gap-3">
              ...
              {detectedWord !== null && (
                <div className={`rounded-2xl border px-4 py-3 text-center ${
                  ...
                  <p className={`mt-1 text-sm font-semibold ${isCorrect ? 'text-green-700' : 'text-red-600'}`}>
