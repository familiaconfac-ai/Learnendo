...
type UiLanguageCode = 'en' | 'pt' | 'es';

// Standardized UI language variable with fallback
const uiLanguage: UiLanguageCode = (localStorage.getItem('learnendo_base_ui_lang') as UiLanguageCode) ?? 'pt';

const getWsl = (): WsLabels => {
  // Use the globally defined uiLanguage constant
  // Fallback to 'pt' is already handled by the uiLanguage constant itself
  // and by the WS_LABELS definition.
  return WS_LABELS[uiLanguage] ?? WS_LABELS.pt;
};
...
const VocabPopup: React.FC<VocabPopupProps> = ({ vocab, userId, onClose }) => {
  ...
  // Use the globally defined uiLanguage constant
  const targetLang = LANG_MM[uiLanguage] ?? 'pt';
...
export const WorkspaceCanvas: React.FC<WorkspaceCanvasProps> = ({
  ...
  if (!userId) {
    ...
    alert('Erro: userId não fornecido. A funcionalidade de salvar/carregar materiais não funcionará.');
  }
  ...
  return (
    <div className="flex flex-col w-full h-full bg-slate-100 overflow-hidden" style={{ fontFamily: 'Arial, sans-serif' }}>
      ...
      >
        ...
        {selected && selected.type === 'text' && !readOnly && (
          <button
            onClick={() => {
              const nextLabel = window.prompt('Nome da caixa de texto', selected.label ?? '');
              if (nextLabel !== null) updateItem(selected.id, { label: nextLabel.trim() });
            }}
            className="h-7 rounded px-2 text-xs text-slate-700 border border-slate-200 hover:bg-slate-100 transition"
            title="Editar nome da caixa"
          >
            {selected.label ? `Caixa: ${selected.label}` : 'Nome da caixa'}
          </button>
        )}
    ...
    <div style={blockStyle} onClick={onSelect}>
      ...
      <div className="absolute inset-x-0 top-0 z-20 px-2 py-1 bg-white/90 border-b border-slate-200 text-[11px] font-semibold text-slate-700">
        <div className="flex items-center justify-between gap-2">
          <span>{item.label?.trim() ? item.label : 'Sem nome'}</span>
          {isLockedByOther && (
            <span className="text-[10px] text-slate-500 font-normal">Editing by {lockOwnerName}</span>
          )}
        </div>
      </div>
      <div
        ref={contentRef}
        contentEditable={!readOnly && !isLockedByOther}
        suppressContentEditableWarning
        spellCheck
        ...
        style={{
          ...
          cursor: readOnly || isLockedByOther ? 'not-allowed' : 'text',
          wordBreak: 'break-word',
          opacity: isLockedByOther ? 0.85 : 1,
        }}
      />
      {isSelected && !readOnly && <ResizeHandle onPointerDown={onPointerDownResize} />}
    </div>
