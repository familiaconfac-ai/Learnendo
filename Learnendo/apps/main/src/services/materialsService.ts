...
export async function saveWorkspaceAsMaterial(
  ...
  console.log('[Materials] saveWorkspaceAsMaterial CALLED with userId:', userId, 'pages:', pages.length, 'title:', options.title);
  ...
  if (auth.currentUser?.uid !== userId) {
    console.warn('[Materials] userId mismatch! Provided:', userId, 'Auth user:', auth.currentUser?.uid, '— proceeding anyway for debugging');
  }
  ...
  console.log(`[Materials] SAVE START — id=${materialId} title="${options.title}" pages=${safePages.length} createdBy=${userId.slice(0, 8)}`);
...
export async function loadMaterialToWorkspace(
  ...
  console.log('[Materials] loadMaterialToWorkspace CALLED with materialId:', materialId, 'classId:', classId, 'uid:', uid);
...
export async function getMaterialsByUser(userId: string): Promise<WorkspaceMaterial[]> {
  console.log('[Materials] getMaterialsByUser CALLED with userId:', userId);
  console.log('[Materials] Auth currentUser:', auth.currentUser?.uid, 'isAnonymous:', auth.currentUser?.isAnonymous);
  ...
  try {
    ...
    console.log(`[Materials] LOAD SUCCESS ✅ — found ${materials.length} materials for user=${userId.slice(0, 8)}`);
