...
export const BottomNavigation: React.FC<BottomNavigationProps> = ({
  ...
  return (
    <div className="bottom-navigation fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-200 flex justify-around items-center py-2 px-1 overflow-x-hidden max-w-full">
      ...
      <button
        onClick={onGrammar}
        className="flex flex-col items-center gap-0.5 px-1.5 sm:px-2 text-xs flex-shrink-0 text-slate-500 hover:text-blue-500 transition-colors"
        title={currentLessonNumber ? `Lesson ${currentLessonNumber} Grammar` : 'Grammar'}
      >
        <span className="text-lg">📖</span>
        <span className="hidden sm:inline">Grammar</span>
        {currentLessonNumber && (
          <span className="-mt-0.5 text-[9px] font-bold text-blue-500">L{currentLessonNumber}</span>
        )}
      </button>
      ...
      <button
        ...
        className={`flex flex-col items-center gap-0.5 px-1.5 sm:px-2 text-xs flex-shrink-0 ${currentSection === 'RANK' ? 'text-blue-500' : 'text-slate-500'}`}
      >
        <span className="text-lg">🏆</span>
