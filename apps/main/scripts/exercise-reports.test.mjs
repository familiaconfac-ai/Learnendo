...
assert.match(dashboard, /publishedDayExerciseSequences/, 'report resolution must check the same published day sequence used by students');
assert.match(dashboard, /loadPublishedDaySequence/, 'report opening must read the public sequence directly');
const reportResolver = dashboard.slice(dashboard.indexOf('const resolveReportedLocation'), dashboard.indexOf('const resolveSelectedFromPublishedSequence'));
assert.doesNotMatch(reportResolver, /getDaySequenceState/, 'opening a report must not depend on canonical or draft sequence reads');
assert.doesNotMatch(reportResolver, /console\.warn/, 'published sequence failures must not silently fall back');
assert.match(dashboard, /Conteúdo no momento da denúncia \(snapshot histórico\)/, 'the historical snapshot must be labelled explicitly');
assert.match(dashboard, /<ExerciseEditorModal key=/, 'switching reports must remount the editor with a source-specific key');
const editor = read(resolve(app, 'components', 'ProblemReports', 'ExerciseEditorModal.tsx'));
assert.match(editor, /hydrateRequestRef/, 'stale editor loads must be guarded by a request generation');
assert.match(editor, /requestId !== hydrateRequestRef\.current/, 'an old response must not overwrite the selected exercise');
assert.match(editor, /setSandboxOpen\(false\)/, 'switching exercises must close and reset the sandbox');
assert.match(editor, /ID solicitado e retornado/, 'the editor must expose requested and returned identities');
assert.match(editor, /Composição da base publicada com o override/, 'the editor must expose base, override and effective content');
assert.match(editor, /display está ausente no override/, 'partial overrides must warn when inherited display conflicts with effective audio');
assert.match(editor, /<ExerciseSandbox key=\{`\$\{effective\.id\}/, 'the sandbox must remount from the current effective exercise');
